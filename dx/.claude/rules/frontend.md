...
- Layout:
  ...
  - `src/routes/` — one file per route: `__root.tsx` (layout + nav `navItems`), `index.tsx`
    ...
    `notifications.tsx` (`/notifications`, the inbox — unread carries the weight, clicking marks
    read and follows the message; the bell with its unread badge lives in `__root.tsx`),
  ...
  - `src/features/notifications/` — the same two files for the inbox: `registry.ts` (a handler
    ...
    `navigate({ to: "/datasets/$datasetId" })`). The bell's count is invalidated after *every*
    successful mutation, once, in `main.tsx`'s `MutationCache` — any write can produce a
    notification, so no feature page has to remember to do it.
