...
def test_patch_replaces_the_whole_tag_set(auth_client: Client, user: User) -> None:
    with acting_as(user):
        dataset = services.create_dataset(user, name="Orders", tags=["sales", "old"])
...
def test_patching_only_tags_does_not_bump_the_dataset(auth_client: Client, user: User) -> None:
    ...
    with acting_as(user):
        dataset = services.create_dataset(user, name="Orders")
...
def test_put_without_tags_clears_them(auth_client: Client, user: User) -> None:
    ...
    with acting_as(user):
        dataset = services.create_dataset(user, name="Orders", tags=["sales"])
...
def test_tags_are_matched_case_insensitively_and_deduplicated(user: User) -> None:
    with acting_as(user):
        first = services.create_dataset(user, name="a", tags=["Sales", " sales ", "SALES"])
        second = services.create_dataset(user, name="b", tags=["sales"])
...
def test_a_retired_tag_name_can_be_used_again(user: User) -> None:
    ...
    with acting_as(user):
        dataset = services.create_dataset(user, name="a", tags=["sales"])
        retired = Tag.objects.get(name="sales")

        services.set_dataset_tags(user, dataset, [])
        assert Tag.objects.filter(name="sales").count() == 0

        services.set_dataset_tags(user, dataset, ["sales"])
...
def test_soft_deleting_a_dataset_takes_its_links_with_it(user: User) -> None:
    with acting_as(user):
        dataset = services.create_dataset(user, name="Orders", tags=["sales"])
        services.delete_dataset(user, DatasetId(dataset.pk))
...
def test_tag_links_are_read_through_the_owned_manager(user: User) -> None:
    ...
    """
    with acting_as(user):
        keeper = services.create_dataset(user, name="Keeps it", tags=["sales"])
        dropper = services.create_dataset(user, name="Drops it", tags=["sales"])
        services.set_dataset_tags(user, dropper, [])
...
def test_tags_are_tenant_isolated(user: User, other_user: User) -> None:
    with acting_as(user):
        services.create_dataset(user, name="mine", tags=["shared-name"])
    with acting_as(other_user):
        services.create_dataset(other_user, name="theirs", tags=["shared-name"])
...
def test_pruning_only_touches_the_callers_tags(user: User, other_user: User) -> None:
    with acting_as(other_user):
        services.create_dataset(other_user, name="theirs", tags=["keep"])
    with acting_as(user):
        dataset = services.create_dataset(user, name="mine", tags=["drop"])
        services.set_dataset_tags(user, dataset, [])
    with acting_as(other_user):
