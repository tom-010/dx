...
Creates apps/<name>/ (owned model, schemas, services, router with paginated list + CRUD incl.
PUT/PATCH, admin, tests), registers it in INSTALLED_APPS and config/api.py (at the `# needle:`
comments) and runs makemigrations. Then: ./scripts/sync_schema.sh for the frontend client.
"""
...
def command(name: str, model: str | None, no_migrations: bool) -> None:
    ...
    console.print(
        Panel(
            ...
            "  2. ./scripts/sync_schema.sh  → typed client in frontend/src/api/\n"
            f"  3. add the resource to RESOURCES in apps/core/tests/test_ownership.py\n"
            f"  4. frontend: src/routes/{spec.name}.tsx + nav entry in __root.tsx",
